...
routingParameter.prototype.resetStartMarker = function () {
...
routingParameter.prototype.resetDestinationMarker = function () {
...
routingParameter.prototype.resetRoute = function () {
    routeController.clear(); // Clear existing route, if any
    routeOptionDisplay = false;
    document.getElementById("routeOptions").style.display = "none";
    document.getElementById("routeExtraParameters").style.display = "none";
    document.getElementById("routeInfo").classList.remove("route-success");
    this.historyPushState();
}
...
routingParameter.prototype.reverseDirection = function() {
...
var mapClick=function(lngLat, zLevel) {
     routingInformation.latestClickPos = {lngLat: lngLat, zLevel: zLevel};
...
var convertSecondsToDisplayTime = function (seconds) {
    ...
    var minutes = Math.ceil((seconds%(60*60))/60);
    return (hours > 0?hours+' hr '+minutes+' min':minutes+' min');
}

var convertMetersToDisplayDistance = function (meters) {
    if (meters >=1000)
    {
...
var processDistance = function (object) {
    var distance = 0;
    routerDescriptionDom = document.getElementById("routeDescription");
    if (object.distanceWalkingMeters > 0){
        distance +=object.distanceWalkingMeters;
        routerDescriptionDom.classList.add('walking');
    } else 
        routerDescriptionDom.classList.remove('walking');
    if (object.distanceDrivingMeters > 0){
        distance +=object.distanceDrivingMeters;
        routerDescriptionDom.classList.add('driving');
    } else 
        routerDescriptionDom.classList.remove('driving');
    document.getElementById('distanceValue').innerHTML=convertMetersToDisplayDistance(distance);
}
...
var setRoute = function(start, dest, avoidStairs){
    ...
    Mazemap.Data.getRouteJSON(start, dest, {'avoidStairs':avoidStairs})
        ...
        document.getElementById('traveltimeValue').innerHTML=convertSecondsToDisplayTime(geojson.properties.pathMetrics.durationEstimateSeconds);
            processDistance(geojson.properties.pathMetrics);
        ...
        document.getElementById("fromLocationBtn").onclick=function(e){
            ...
            if (navigator.geolocation) {
				...
				fetch(MAZEMAP_LIST_LIPI_URL).then(function(response) {
					...
					if (json){
                        maptrack.event('geoLocate','foundlipi',document.location.href);
						...
						fetch(LIPI_URL).then(function(response) {
							...
							if (json){
								...
								console.log("Correct");
                                maptrack.event('geoLocate','returnlipi', {lat: lat, lng: lng});
...
var deviceGeolocation = function (point) {
	...
	navigator.geolocation.getCurrentPosition(
		function(pos) {
            maptrack.event('geoLocate','returndeviceloc',{lat: pos.coords.latitude, lng: pos.coords.longitude});
			var start = false;
			if (point == 'from')
				start = true;
...
var updateDisplayInformation = function(domID, position, customizeMessage) {
    ...
    if (domDocument) {
        if (customizeMessage)
            domDocument.innerHTML=customizeMessage;
        else if (position.hasOwnProperty('poiId')) {
                Mazemap.Data.getPoi(position.poiId).then( function(poi) {
                    ...
                    html = '<h3>'+ (poi.properties.title!=='null'?poi.properties.title:'')+'</h3><p>'+locationHTML+'</p>';
                    domDocument.innerHTML = html;
                }).catch( function(e) {
                    domDocument.innerHTML='<p>Corresponding POI cannot be found</p>';
                });
            } else {
                domDocument.innerHTML='<p>Selected location</p>';
            }
        domDocument.parentNode.classList.add('selected');
    }
};
...
document.getElementById("reverseDirection").onclick=function(e) {
    e.preventDefault();
    routingInformation.reverseDirection();
...
var resetLinks = document.getElementsByClassName('clear-point');
for (var i=0; i < resetLinks.length; i++) {
    resetLinks[i].onclick = function(e){
        e.preventDefault();
        this.previousElementSibling.innerHTML = '<a href="#" id="' + this.previousElementSibling.id+ 'Btn">Use my location</a>, or click on the map.';
        this.parentNode.classList.remove("selected");
        setGeolocationButton();
        if (this.previousElementSibling.id == 'fromLocation')
            routingInformation.resetStartMarker();
        else if (this.previousElementSibling.id == 'toLocation')
